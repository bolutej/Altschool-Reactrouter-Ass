...
const User = () => {
    ...
    const getProfile = async(page) => {
        setIsloading(true)
        const data = await fetch(`https://randomuser.me/api/?page=${page}&seed=4d1a9d8ef9f34996`);
  ...
  return (
    <div className='pages'>
      <button disabled={currentPage === 1} onClick={()=>setCurrentPage(currentPage - 1)}>Prev</button>
      {
        [...Array(pages).keys()].map((_, index)=>(
            <button 
                ...
                className='buttons'
                style={{ 
                    backgroundColor:(index + 1) === currentPage ? "black": "transparent",
      ...
      <Contact user={user} />
      <p>Pages: {currentPage} of {pages}</p>
